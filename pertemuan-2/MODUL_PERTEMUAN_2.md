...
Pada akhir praktikum ini, mahasiswa akan punya **2 project tambahan**:

1. **`profile_page`** — halaman profil dengan `Scaffold` lengkap: AppBar, Drawer, FAB, BottomNavigationBar, body yang bisa di-scroll
2. **`widget_gallery`** — aplikasi katalog widget berisi 5 halaman kategori (Display, Input, Button, Feedback, Layout)
...
Tutup `profile_page` (tekan `q`), buat project kedua untuk eksplorasi widget:
...
Project ini akan punya **menu utama** dengan 5 tombol kategori. Setiap tombol menuju halaman katalog widget.

### 5.1 Setup Menu Utama

Ganti seluruh `lib/main.dart`:
...
Hot restart. Anda akan lihat 5 kartu kategori. Tap salah satu → halaman placeholder.

> 💡 Baris `Navigator.push(...)` adalah **pengenalan navigasi**. Kita akan dalami di Praktikum 02. Untuk sekarang, cukup paham: ini cara berpindah halaman.

### 5.2 Isi Kategori (Pilih 2 dari 5 — sesuai waktu)

Ganti `CategoryPage` agar menampilkan widget berbeda per kategori. Tambahkan switch sederhana:
...
- [ ] Project `widget_gallery` jalan dengan 5 kategori, minimal 2 kategori sudah dieksplorasi tuntas
